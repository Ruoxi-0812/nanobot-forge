...
from nanobot.agent.tools.git_tool import GitTool
from nanobot.agent.tools.lint import LintTool
from nanobot.agent.tools.package_manager import PackageManagerTool
from nanobot.agent.tools.python_sandbox import PythonSandboxTool
from nanobot.agent.tools.run_code import RunCodeTool
from nanobot.agent.tools.run_tests import RunTestsTool
...
class AgentLoop:
    ...
    def __init__(
        ...
        if _tc.forge.enable:
            self.tools.register(GitTool(workspace=str(self.workspace)))
            self.tools.register(LintTool(workspace=str(self.workspace)))
            self.tools.register(PackageManagerTool(
                workspace=str(self.workspace),
                python_executable=_tc.forge.python_executable,
            ))
            self.tools.register(RunCodeTool(
                workspace=str(self.workspace),
                python_executable=_tc.forge.python_executable,
                timeout=_tc.forge.run_code_timeout,
            ))
            self.tools.register(RunTestsTool(
                workspace=str(self.workspace),
                python_executable=_tc.forge.python_executable,
                timeout=_tc.forge.run_tests_timeout,
            ))
            self.tools.register(PythonSandboxTool(
                blocked_imports=_tc.forge.sandbox.blocked_imports,
                timeout=_tc.forge.sandbox.timeout,
            ))
