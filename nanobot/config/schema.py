...
class PythonSandboxConfig(Base):
    """Sandboxed Python snippet execution configuration."""
...
class ForgeConfig(Base):
    """nanobot-forge coding agent tool configuration."""

    enable: bool = True
    python_executable: str = "python"  # Python interpreter used by run_code and run_tests
    run_code_timeout: int = Field(default=30, ge=1, le=300)   # default timeout for run_code (seconds)
    run_tests_timeout: int = Field(default=120, ge=1, le=600)  # default timeout for run_tests (seconds)
    sandbox: PythonSandboxConfig = Field(default_factory=PythonSandboxConfig)
